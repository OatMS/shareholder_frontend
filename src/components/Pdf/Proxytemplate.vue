<template lang="html">
  <page size="A4">
    <!-- <v-layout class="pl-3 pr-4">
    <v-spacer></v-spacer>
    <v-btn small>พิมพ์</v-btn>
    <v-btn small>ยกเลิก</v-btn>
  </v-layout> -->
  <div>
    <table align="right" style="margin-bottom: 20px;">
      <tr>
        <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
          <span style="font-weight: bold; padding-top: 0px">4010008296</span>
        </th>
        <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
      </tr>
      <tr>
        <td>
          วาระที่ 4
        </td>
        <td style="text-align: left;" class="tablea">
          พิจารณาอนุมัติงบการเงินประจำปีสิ้นสุด ณ วันที่ 31 ธันวาคม 2560 ซึ่งผ่านการตรวจสอบจากผู้สอบบัญชี
        </td>
      </tr>
      <tr>
        <td rowspan="2">
          <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
          <span>4010008296</span>
          <span style="font-weight: bold;">เลขอ้างอิง</span>
          <span>1001</span>
        </td>
      </tr>
      <td style="height: 80px; table-layout: auto;">
        <!-- <span style="visibility: hidden;">lineup</span> -->
        <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
          <img class="imgckbox" src="/static/img/icons/ckbox.gif">
          เห็นด้วย
        </span>
        <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
          <img class="imgckbox" src="/static/img/icons/ckbox.gif">
          ไม่เห็นด้วย
        </span>
        <span style="padding-top: 10px; text-align: right">
          <img class="imgckbox" src="/static/img/icons/ckbox.gif">
          งดออกเสียง
        </span>
      </td>
      <td style="text-align: left;" class="tablea">
        <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
        <span style="text-align: left">ชื่อผู้มอบฉันทะ</span>
        <span style="text-align: left;">จำนวนหุ้น ____________________</span>
        <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
        <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px; padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
      </td>
    </tr>
  </table>
  <table style="margin-top: 20px; margin-bottom: 20px;">
    <tr>
      <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
        <span style="font-weight: bold; padding-top: 0px">4010008296</span>
      </th>
      <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
    </tr>
    <tr>
      <td>
        วาระที่ 2
      </td>
      <td style="text-align: left;" class="tablea">
        พิจารณารับรองรายงานการประชุมสามัญผู้ถือหุ้นประจำปี 2560 ซึ่งประชุมเมื่อวันที่ 21 เมษายน 2560
      </td>
    </tr>
    <tr>
      <td rowspan="2">
        <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
        <span>4010008296</span>
        <span style="font-weight: bold;">เลขอ้างอิง</span>
        <span>1001</span>
      </td>
    </tr>
    <td style="height: 80px; table-layout: auto;">
      <!-- <span style="visibility: hidden;">lineup</span> -->
      <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
        <img class="imgckbox" src="/static/img/icons/ckbox.gif">
        เห็นด้วย
      </span>
      <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
        <img class="imgckbox" src="/static/img/icons/ckbox.gif">
        ไม่เห็นด้วย
      </span>
      <span style="padding-top: 10px; text-align: right">
        <img class="imgckbox" src="/static/img/icons/ckbox.gif">
        งดออกเสียง
      </span>
    </td>
    <td style="text-align: left;" class="tablea">
      <span style="text-align: left;">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
      <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
      <span style="text-align: left;">จำนวนหุ้น ____________________</span>
      <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
      <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
    </td>
  </tr>
</table>
<table align="right" style="margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 6.1
    </td>
    <td style="text-align: left;" class="tablea">
      พิจารณาเลือกตั้งกรรมการแทนกรรมการที่ออกตามวาระประจำปี 2561 นายมรกต เธียรมนตรี
    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
<table style="margin-top: 20px; margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 5
    </td>
    <td style="text-align: left;" class="tablea">
      พิจารณาจัดสรรกำไรสุทธิเป็นทุนสำรองตามกฎหมายและการจ่ายเงินปันผลสำหรับผลประกอบการประจำปี 2560
    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
<table align="right" style="margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 6.3
    </td>
    <td style="text-align: left;" class="tablea">
      พิจารณาเลือกตั้งกรรมการแทนกรรมการที่ออกตามวาระประจำปี 2561 นางมรกต กุลธรรมโยธิน
    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
<table style="margin-top: 20px; margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 6.2
    </td>
    <td style="text-align: left;" class="tablea">
      พิจารณาเลือกตั้งกรรมการแทนกรรมการที่ออกตามวาระประจำปี 2561 นายอนิรุทธ์ หิรัญรักษ์
    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
<table align="right" style="margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 7
    </td>
    <td style="text-align: left;" class="tablea">
      พิจารณาอนุมัติการกำหนดค่าตอบแทนกรรมการ
    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
<table style="margin-top: 20px; margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 6.4
    </td>
    <td style="text-align: left;" class="tablea">
      พิจารณาเลือกตั้งกรรมการแทนกรรมการที่ออกตามวาระประจำปี 2561 นางสาวนฤมล วังศธรธนคุณ
    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น _________ </span>
    <span style="text-align: left">ลงชื่อ _____________ (ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
<table align="right" style="margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 9
    </td>
    <td style="text-align: left;" class="tablea">
      พิจารณาอนุมัติการแก้ไขข้อบังคับ บริษัท อินเทอร์เน็ตประเทศไทย จำกัด(มหาชน) ข้อ 16
    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;"></span> -->
  </td>
</tr>
</table>
<table style="margin-top: 20px; margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 8
    </td>
    <td style="text-align: left;" class="tablea">
      พิจารณาแต่งตั้งผู้สอบบัญชีและกำหนดค่าสอบบัญชีประจำปี 2561
    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
<table align="right" style="margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 11
    </td>
    <td style="text-align: left;" class="tablea">

    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
<table style="margin-top: 20px; margin-bottom: 20px;">
  <tr>
    <th rowspan="2"><img class="img" src="/static/img/icons/qrtest.png">
      <span style="font-weight: bold; padding-top: 0px">4010008296</span>
    </th>
    <th colspan="3">บัตรลงคะแนนการประชุมสามัญผู้ถือหุ้น ประจำปี 2561</th>
  </tr>
  <tr>
    <td>
      วาระที่ 10
    </td>
    <td style="text-align: left;" class="tablea">

    </td>
  </tr>
  <tr>
    <td rowspan="2">
      <p style="margin-bottom: 0px; font-weight: bold;">เลขทะเบียน</p>
      <span>4010008296</span>
      <span style="font-weight: bold;">เลขอ้างอิง</span>
      <span>1001</span>
    </td>
  </tr>
  <td style="height: 80px; table-layout: auto;">
    <!-- <span style="visibility: hidden;">lineup</span> -->
    <span style="padding-top: 5px; padding-bottom: 5px; text-align: right;">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      เห็นด้วย
    </span>
    <span style="padding-top: 10px; padding-bottom: 5px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      ไม่เห็นด้วย
    </span>
    <span style="padding-top: 10px; text-align: right">
      <img class="imgckbox" src="/static/img/icons/ckbox.gif">
      งดออกเสียง
    </span>
  </td>
  <td style="text-align: left;" class="tablea">
    <span style="text-align: left">วีระ เดชธำรง โดยบริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)</span>
    <span style="text-align: left;">ชื่อผู้มอบฉันทะ</span>
    <span style="text-align: left;">จำนวนหุ้น ____________________</span>
    <span style="text-align: left">ลงชื่อ ________________________(ผู้มอบฉันทะ)</span>
    <!-- <span style="text-align: left; font-size:9px; padding-bottom: 5px;padding-left: 20px;">(ผู้มอบฉันทะ)</span> -->
  </td>
</tr>
</table>
</div>
</page>
</template>

<script>
export default {
}
</script>

<style lang="css">
page[size="A4"] {
  background: white;
  width: 21cm;
  height: 29.7cm;
  display: block;
  margin: 0 auto;
  margin-bottom: 0.5cm;
  box-shadow: 0 0 0.5cm rgba(0,0,0,0.5);
}
@media print {
  body, page[size="A4"] {
    margin: 0;
    box-shadow: 0;
  }
}
table {
  border-collapse: collapse;
  margin-left: 30px;
  margin-right: 30px;
}

table, th, td {
  border: 1px solid black;
  /* table-layout: auto; */
}

th, td {
  padding-left: 10px;
  padding-right: 10px;
  text-align: center;
  font-size:9px;
}
span {
  padding-top: 6px;
  text-align: center;
  display: block;
  font-size:8px;
}
.img {
  height: 50px;
}
.tablea {
  table-layout: auto;
  width: 180px;
  height: 10px;
}
.imgckbox {
  top:-5px;
  width:18px;
  height:18px;
  float: left;
  z-index:10;
  position:relative;
}
.tableblock {
  display: block;
}
</style>
